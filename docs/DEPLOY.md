...
> `ddobakddobak.harvester.kr` 로 HOPE 서비스(프론트 + 백엔드 API + AI 추론) 전부를 하나의 도메인으로 배포. 이미 뜬 [Harvester 서버 · Cloudflare Tunnel 인프라](../../Game/docs/DEPLOY.md)를 재사용한다.
...
[Harvester 10.3.10.168 · Ubuntu · Docker · GPU 없음]
     ...
     └─ cloudflared 컨테이너 (dduckyee 스택 공용, host network)
         └─ ingress: ddobakddobak.harvester.kr → localhost:8094
     ▼
[Cloudflare 엣지] ──▶ https://ddobakddobak.harvester.kr
```
...
### 2.1 Cloudflare 터널에 ddobakddobak.harvester.kr 라우팅 추가
...
3. Save → DNS CNAME(`ddobakddobak.harvester.kr` → `55cbfbbf-...cfargotunnel.com`, proxied)이 자동 생성됨
...
# { "hostname": "ddobakddobak.harvester.kr", "service": "http://localhost:8093" }
# 그리고 DNS CNAME 생성 (zone_id = 30e78bc81598c995f8f041a35b975e28)
```

⚠️ **harvester.kr 은 공유 존.** 다른 서비스 레코드는 절대 건드리지 말 것. `dig +short ddobakddobak.harvester.kr` 이 비어 있는지 확인 후 진행 (2026-07 현재 비어 있음).
...
```bash
# 외부에서 헬스 체크
curl -s https://ddobakddobak.harvester.kr/health | jq .
# 기대:
# {
#   "ok": true,
...
# 데모 계정 로그인
curl -s -X POST https://ddobakddobak.harvester.kr/api/auth/login \
...
Swagger UI: <https://ddobakddobak.harvester.kr/docs>
...
cat > .env.local <<'EOF'
VITE_API_BASE_URL=https://ddobakddobak.harvester.kr
EOF

npm run dev   # 프론트가 ddobakddobak.harvester.kr/api/* 로 붙는다
```

**CORS**: `ddobakddobak.harvester.kr` 이 다른 오리진(로컬 프론트)에서 호출될 때 CORS 필요. `deploy/.env` 의 `CORS_ORIGIN` 값을 프론트 오리진으로 좁혀 두면 안전:
```env
...
### 배포는 됐는데 라이브에 반영 안 됨
- 컨테이너 교체에 몇 초 필요. `curl -s https://ddobakddobak.harvester.kr/health` 를 몇 번 재시도.
- 강제 재빌드: `./deploy/harvester.sh` (이미 --build 옵션 포함)

### `CORS error` — 프론트가 백엔드를 못 부름
- `deploy/.env` 의 `CORS_ORIGIN` 값이 프론트 오리진과 일치하는지 확인
- 응답 헤더 검증: `curl -I https://ddobakddobak.harvester.kr/health | grep -i access-control`
...
- **왜 ddobakddobak.harvester.kr?** Game 인프라의 Cloudflare Tunnel(55cbfbbf) · cloudflared 컨테이너를 그대로 재사용. HOPE 는 컨테이너 하나만 추가.
